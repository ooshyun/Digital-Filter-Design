...
def plot_cascade_sheving_filter():
    ...
    fticklabels = ['15.','31.', '62.','125', '250', '500', '1k', '2k', '4k','8k', '16k']
    ...
    for n in n_list:
        ...
        biquad_per_octave, num_biquad, Gb, G = shelving_filter_parameters(biquad_per_octave=biquad_per_octave,
                                                                        Gd=G_desire,
                                                                        slope=slope, 
                                                                        BWd=BWd)
        ...
        sos_sdomain = low_shelving_2nd_cascade(w0, Gb, num_biquad, biquad_per_octave, Q=Q)
...
def plot_sheving_filter_digital():
    ...
    for boost in range(-24, 0, 3):
        b, a = shelf(Wn=wn, dBgain=boost, S=slide, btype=btype, ftype=ftype,
                        analog=False, output='ba')
        w, h = freqz(b, a, frame_size)
        freq=w * fs * 1.0 / (2 * np.pi)
    ...
    plt.xlim(0.1, 1000)
    plt.xticks([20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000], ["10", "50", "100", "200", "500", "1K", "2K", "5K", "10K", "20K"])
...
def iir_filter_mulitstage_process_cascade(fc_band):
    ...
    wave_processor.run(savefile_path=result_path+'/whitenoise_3peak_250_2000_8000.wav')
...
def fi_example():
    ...
    print("-"*10)
    if(x*b - mul_x_b < 10e-3):print(f'PASS, {mul_x_b}')
    ...
    print("-"*10)
    if(y*y - mul_y_a < 10e-3):print(f'PASS, {mul_y_a}')
    ...
    print("-"*10)
    if(zi_predict - zi < 10e-3):print(f'PASS, {zi}')
...
def iir_filter_floating_format():
    ...
    a_local = np.array(fi(Values=alpha,
                Signed=1,
                TotalLen=32,
                FracLen=30,
                Format=0,
                ReturnVal='Dec'))
    b_local = np.array(fi(Values=beta,
                Signed=1,
                TotalLen=32,
                FracLen=30,
                Format=0,
...
def lowpass_floating_cvt2binary32():
    coeff = [0x5a9df7ac,0x4026e73d,0x2d6a8670,0x2026f310,0x16c310e4,0x101d3f2e,0x0b68737a,0x08138562,0x05b7b15b,0x040c3714,0x02dd958b,0x0207567b,0x016fa9bb,0x01044915,0x00b8449d,0x008273a7,0x005c5a50,0x0041617a,0x002e493a,0x0020c49c,0x001732ae,0x00106c44,0x000ba064,0x00083b20,0x0005d3bb,0x00042011,0x0002eba3,0x0002114a,0x000176b5,0x00010946,0x0000bbcd,0x000084f4,0x00005e20,0x000042a3,0x00002f2d,0x00002166,0x000017a5,0x000010bd,0x00000bda,0x00000864,0x000005f1,0x00000435,0x000002fa,0x0000021c,0x0000017e,0x0000010f,0x000000c0,0x00000088,0x00000060,0x00000044,0x00000031,0x00000023,0x00000019,0x00000012,0x0000000d,0x00000009,0x00000007,0x00000005,0x00000004,0x00000003,0x00000002,0x00000002,0x00000001,0x00000000,0,0,0,0,0,0,0,0]
...
def test_hilber_from_scratch_time_domain():
    ...
    for n in range(N):
        print('{:2d} {:+5.2f} {:+10.2f} {:+5.2f} {:+5.2f}'.format(n, y[n], z1[n], z2[n], z3[n]))
...
def test_hilber_from_scratch_frequency_response():
    ...
    y_HT2 = np.fft.irfft(y_fft_real[:len(y_fft_real//2+1)], n=len(y_fft_real))
...
def iir_filter_multistage_process_parallel(file):
    ...
    coeff_text, bias = np.array(coeff_text[:-1], dtype=np.float64), np.array(coeff_text[-1]) 
    coeff = [coeff_text, bias]

    wave_processor.filters = coeff

    outresult_path=result_path+'/whitenoise'+file.split('.')[-2].split('/')[-1]+'.wav'
...
def plot_audacity_freq_response():
    ...
    plt.xticks([10, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000],
