...
def plot_tranfer_response(xaxis, **kwargs):
    ...
    if numPlot == 1:
        ...
        axes[0].plot(xaxis, plot_list[0][origin:endX], plotStyle, label=plot_list[-1].name)
        axes[1].plot(xaxis, plot_list[1][origin:endX], plotStyle, label=plot_list[-1].name)

    else:
        axes = fig.subplots(ncols=numPlot, nrows=2)
        for index_row, axes_rows in enumerate(axes):
            for index_col, ax in enumerate(axes_rows):
                ax.plot(xaxis[origin:endX], plot_list[index_row][index_col][origin:endX], plotStyle, label=plot_list[-1][index_col].name)
                ax.grid()
...
def plot_time_domain(xaxis, **kwargs):
    ...
    if numPlot == 1:
        ...
        axes[0].plot(xaxis[origin:endX], plot_list[0][origin:endX], plotStyle, label=plot_list[-1].name)

    else:
        axes = fig.subplots(ncols=numPlot, nrows=1)
        for index_row, ax in enumerate(axes):
            ax.plot(xaxis[origin:endX], plot_list[0][index_row][origin:endX], plotStyle, label=plot_list[-1][index_row].name)
            ax.grid()
            ax.legend(loc='lower center')
...
def plot_time_domain_compare(xaxis, **kwargs):
    ...
    for index, yaxis in enumerate(plot_list[0]):
        axes.plot(xaxis[origin:endX], yaxis[origin:endX], plotStyle, label=plot_list[-1][index].name)
...
def plot_transfer_response_compare(xaxis, **kwargs):
    ...
    axes = fig.subplots(ncols=1, nrows=2)
    for index, yaxis in enumerate(plot_list[0]):
        axes[0].plot(xaxis[origin:endX], yaxis[origin:endX], plotStyle, label=plot_list[-1][index].name)
    axes[0].legend(loc='upper right')
    axes[0].grid()

    for index, yaxis in enumerate(plot_list[1]):
        axes[1].plot(xaxis[origin:endX], yaxis[origin:endX], plotStyle, label=plot_list[-1][index].name)
